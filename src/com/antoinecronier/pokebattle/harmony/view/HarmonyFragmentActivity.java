/**************************************************************************
 * HarmonyFragmentActivity.java, pokebattle Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
 ...
 *
 **************************************************************************/
package com.antoinecronier.pokebattle.harmony.view;
...
import com.antoinecronier.pokebattle.menu.PokebattleMenu;
import com.antoinecronier.pokebattle.PokebattleApplication;
import com.antoinecronier.pokebattle.PokebattleApplicationBase.DeviceType;
...
public abstract class HarmonyFragmentActivity extends ActionBarActivity {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        try {
            PokebattleMenu.getInstance(this).clear(menu);
            PokebattleMenu.getInstance(this).updateMenu(menu, this);
        } catch (Exception e) {
            e.printStackTrace();
            result = false;
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        boolean result;
        try {
            result = PokebattleMenu.getInstance(this).dispatch(
                                                                    item, this);
        } catch (Exception e) {
            e.printStackTrace();
            result = false;
    ...
    protected void onActivityResult(int requestCode, int resultCode,
                                                                  Intent data) {
        try {
            PokebattleMenu.getInstance(this).onActivityResult(
    ...
    public boolean isDualMode() {
        return PokebattleApplication.getDeviceType(this).equals(DeviceType.TABLET);
    }
}
