/**************************************************************************
 * PokeNpcListActivity.java, pokebattle Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display PokeNpc entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a PokeNpc list. A click on an item will get 
 * you to PokeNpcShowActivity.
 * - On a tablet, the PokeNpcShowFragment will be displayed right next to
 * the list.
 */
public class PokeNpcListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected PokeNpcListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected PokeNpcShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (PokeNpcShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (PokeNpcListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    protected int getContentView() {
        return R.layout.activity_pokenpc_list;
    }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, PokeNpcShowActivity.class);
            final PokeNpc item = PokeNpcContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(PokeNpcContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(PokeNpc item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            PokeNpc item = PokeNpcContract.cursorToItem(
