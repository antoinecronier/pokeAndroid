/**************************************************************************
 * PokeObjetListActivity.java, pokebattle Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display PokeObjet entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a PokeObjet list. A click on an item will get 
 * you to PokeObjetShowActivity.
 * - On a tablet, the PokeObjetShowFragment will be displayed right next to
 * the list.
 */
public class PokeObjetListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected PokeObjetListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected PokeObjetShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (PokeObjetShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (PokeObjetListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    protected int getContentView() {
        return R.layout.activity_pokeobjet_list;
    }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, PokeObjetShowActivity.class);
            final PokeObjet item = PokeObjetContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(PokeObjetContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(PokeObjet item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            PokeObjet item = PokeObjetContract.cursorToItem(
