/**************************************************************************
 * PokePositionListActivity.java, pokebattle Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display PokePosition entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a PokePosition list. A click on an item will get 
 * you to PokePositionShowActivity.
 * - On a tablet, the PokePositionShowFragment will be displayed right next to
 * the list.
 */
public class PokePositionListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected PokePositionListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected PokePositionShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (PokePositionShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (PokePositionListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    protected int getContentView() {
        return R.layout.activity_pokeposition_list;
    }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, PokePositionShowActivity.class);
            final PokePosition item = PokePositionContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(PokePositionContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(PokePosition item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            PokePosition item = PokePositionContract.cursorToItem(
